...
if (!pgUser || !pgPassword || !pgDbName) {
    console.error("Error: Missing required PostgreSQL environment variables (DB_USERNAME, DB_PASSWORD, AUTHORIZATION_DB_NAME) in .env");
    process.exit(1);
}
...
console.log(`Using PostgreSQL URL: postgresql://${pgUser}:***@${pgHost}:${pgPort}/${pgDbName}?schema=public`); // Log without password
...
async function migrateTable(
  ...
  try {
    ...
    const [countResult]: any = await mysqlConn.query(`SELECT COUNT(*) as count FROM ${tableName}`);
    ...
    while (true) {
      ...
      const dataToInsert = rows.map((row: any) => {
        ...
        for (const mysqlCol in columnMapping) {
          ...
          } else if (typeof row[mysqlCol] === 'number' && (prismaField.endsWith('At') || prismaField.endsWith('Date'))) {
      ...
      try {
        ...
        console.error(`Error inserting batch ${batchCounter} into ${String(prismaModel)}:`, prismaError);
        console.error('Failed batch data:', JSON.stringify(dataToInsert, null, 2));
        throw new Error(`Stopping migration due to error in ${String(prismaModel)} batch insertion.`);
      }
      ...
      if (batchCounter % 100 === 0 || offset >= totalCount) {
           ...
           console.log(`Progress for ${tableName}: ${totalMigrated}/${totalAttempted} attempted (${totalCount} total source), ${currentSkipped} skipped (Batch ${batchCounter})`);
      }
  ...
  console.log(`--- Finished migrating ${tableName}. Attempted: ${totalAttempted}. Migrated: ${totalMigrated}. Skipped: ${finalSkipped}. ---`);
  ...
  try {
    ...
    if (pgSafeTableName === 'permission_policy') {
        console.log(`Skipping sequence reset for table "${pgSafeTableName}" as its ID is not an integer sequence.`);
    } else {
      ...
      console.log(`Attempting to reset ID sequence for table "${pgSafeTableName}"...`);
    ...
    const targetTableName = typeof pgSafeTableName !== 'undefined' ? pgSafeTableName : tableName;
    console.warn(`Warning: Could not automatically reset ID sequence for table "${targetTableName}". Manual check/reset might be needed. Error: ${seqError.message}`);
  }
...
async function main() {
  ...
  try {
    ...
    console.log("MYSQL_PASSWORD:", process.env.MYSQL_PASSWORD ? '***' : 'undefined'); // Don't log password
    ...
    // 2. Group
    await migrateTable(mysqlConn, '`group`', 'group', { // Note backticks for reserved keyword
      id: 'id',
      name: 'name',
